...
func TestIntegration_OneConsumerRollbackSuccess(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
...
func TestIntegration_TwoConsumersFirstRollbackError(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	...
	workerTask2 := rmqworker.WorkerTask{
		...
		Callback: func(w *rmqworker.RMQWorker, h rmqworker.RMQDeliveryHandler) {
			...
			assert.ErrorIs(t, err, cstx.ErrCancelled)
...
func TestIntegration_TwoConsumersSecondRollbackError(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
